...
class AddDataForm extends Component{
    ...
    list_it=(data,parent)=>{
        ...
        for (var c in data) {
            if(level===1){
                ro.push(this.list_to_html(data[parseInt(c)],1,parte));
            }
            else if(level ===2){
                ro.push(this.list_to_html(data[c]["test_name"],1,null));
            }
            else{
                ro.push(this.list_to_html(data[c]['name'],1, null));
            }
        }
        return ro
    }
    ...
    fetch_child=(datas)=>{
        ...
        for(let data in datas){
            ...
            if(data==="imaging_reports"){
                finalrow.push(this.list_it(datas[data]["parameters"][0]["test_names"],1))
            }
            else if(data==="testing_reports"){
                finalrow.push(this.list_it(datas[data]["parameters"][0]["test_names"],2))
            }
            else{
