...
class AddDataForm extends Component{
    ...
    fetch_child=(datas)=>{
        ...
        for(let data in datas){
            console.log(datas[data]["parameters"])
            finalrow.push(<h1>{data}</h1>)
            if(data==="imaging_reports"){
                finalrow.push(this.listit(datas[data]["parameters"][0]["test_names"],0))
            }
            if(data==="testing_reports"){
                finalrow.push(this.listit(datas[data]["parameters"]["test_names"],0))
            }
            else{
